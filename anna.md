...
- `ANNA_VAULT_TOKEN` (optional when using AppRole auth)
- `ANNA_VAULT_ROLE_ID` (required with `ANNA_VAULT_SECRET_ID` when not using token auth)
- `ANNA_VAULT_SECRET_ID` (required with `ANNA_VAULT_ROLE_ID` when not using token auth)
- `ANNA_VAULT_AUTH_PATH` (optional, default `auth/approle/login`)
